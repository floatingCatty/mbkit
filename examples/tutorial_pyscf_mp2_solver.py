"""Formal tutorial for operator construction and PySCF-backed MP2.

This example uses the same explicit four-site Hubbard chain as the DMRG
tutorial, but routes it through the method-based `MP2Solver` facade. The
example solves the two-electron `n_up = n_down = 1` sector, not the half-filled
four-electron chain. Because unrestricted MP2 is non-variational, this
strongly correlated benchmark can produce a total energy below the matching ED
result even when the Hamiltonian mapping is correct. The stable MP2 API exposes
both reference and post-HF one-body observables, so the example demonstrates
the required `kind=` selectors explicitly:

1. define an `ElectronicSpace`,
2. construct the Hamiltonian explicitly from operator terms,
3. solve a fixed-particle problem with `MP2Solver`,
...
from mbkit import ElectronicSpace, LineLattice, MP2Solver
...
def main() -> None:
    ...
    print("PySCF MP2 tutorial: four-site Hubbard chain")
    print("=" * 72)
    print("Space:", space)
    ...
    solver = MP2Solver().solve(hamiltonian, n_electrons=2)
    ...
    print("MP2 exposes one-body observables with explicit kinds; `docc()` is not available.")
    print("UMP2 is non-variational, so its energy may lie below the ED result on this problem.")
    print()
