...
`mbkit` exposes method-based solvers such as `EDSolver`, `DMRGSolver`, `UHFSolver`, `MP2Solver`, `CCSDSolver`, and `CCSDTSolver` on the new operator frontend. `PySCFSolver` remains available as a compatibility umbrella for explicit `method="uhf"`, `method="mp2"`, `method="ccsd"`, or `method="ccsd(t)"` routing. Optional OpenFermion and Qiskit Nature bridges live under `mbkit.operator.interop`.
