...
from mbkit import DMRGSolver, EDSolver, ElectronicSpace, LineLattice, PySCFSolver
...
def test_pyscf_solver_uhf_and_mp2_run_live_two_site_hubbard_problem():
    ...
    assert np.isclose(np.trace(mp2_solver.rdm1()), 2.0, atol=1e-8)
    assert uhf_solver.rdm1().shape == (space.num_spin_orbitals, space.num_spin_orbitals)
    assert mp2_solver.rdm1().shape == (space.num_spin_orbitals, space.num_spin_orbitals)
