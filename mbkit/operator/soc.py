...
def lm2cubic_mat(cubic_mag_num, lm_mag_num, device='cpu'):
    ...
    t2 = 2.0
    s2_1=1.0/np.sqrt(t2)
    M = np.zeros((len(cubic_mag_num),len(cubic_mag_num))) + 0j
...
def get_matrix_lmbasis(basis, device='cpu'):
...
def get_soc_matrix_cubic_basis(orbital: str,cubic_mag_num=None, lm_mag_num=None):  
    ...
    Msoc_lm_clx = Msoc_lm + 0j

    trans = np.kron(Mtrans, np.eye(2)).T
    transHT = trans.T.conj()
    ...
    Msoc_updn_block = np.zeros((2*norbs,2*norbs)) + 0j
